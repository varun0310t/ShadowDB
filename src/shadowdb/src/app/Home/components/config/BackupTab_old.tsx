    ...
    <div className="space-y-3 sm:space-y-4 w-full overflow-hidden">
      <Card className="bg-[#151923] border-gray-800">
        <CardHeader className="p-3 sm:p-4 md:p-6">
          <CardTitle className="text-base sm:text-lg text-white">Backup Configuration</CardTitle>
        ...
        <CardContent className="space-y-3 sm:space-y-4 p-3 sm:p-4 md:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="space-y-1">
              <Label className="text-sm text-gray-200">Automated Backups</Label>
          ...
          <div className="space-y-2">
            <Label className="text-sm text-gray-200">Backup Frequency</Label>
            <FeaturePreview message="Use Manual Backups for now this feature might be available in the future">
            <Select value={frequency} onValueChange={setFrequency} disabled={true} >
              <SelectTrigger className="bg-[#0B0F17] border-gray-800 w-full text-white h-9 text-sm">
          ...
          <div className="space-y-2">
            <Label className="text-sm text-gray-200">Retention Period (days)</Label>
            <FeaturePreview message="Use Manual Backups for now this feature might be available in the future">
            <Input
              type="number"
              value={retention}
              ...
              max="365"
              disabled={true}
              className="bg-[#0B0F17] border-gray-800 w-full text-white h-9 text-sm"
            />
            </FeaturePreview>
          </div>
          ...
          <div className="pt-2 sm:pt-4">
            <Button
              className="bg-purple-600 hover:bg-purple-700 w-full sm:w-auto text-sm py-2"
            ...
            >
              {isCreatingBackup ? (
                <>
                  ...
                  <span className="hidden sm:inline">Creating Backup...</span>
                  <span className="sm:hidden">Creating...</span>
                </>
              ) : (
                <>
                  <RefreshCw className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-2" />
                  <span className="hidden sm:inline">Create Manual Backup</span>
                  <span className="sm:hidden">Create Backup</span>
                </>
              )}
            </Button>
          </div>
      ...
      <Card className="bg-[#151923] border-gray-800">
        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 sm:p-4 md:p-6">
          <div>
            <CardTitle className="text-base sm:text-lg text-white">Recent Backups</CardTitle>
            <CardDescription className="text-xs sm:text-sm text-gray-400">
          ...
          >
            <RefreshCw
              className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${isLoading ? "animate-spin" : ""} sm:mr-0`}
            />
              ...
              <table className="w-full caption-bottom text-xs sm:text-sm min-w-[600px]">
                <thead className="bg-[#0B0F17]">
                  <tr className="border-b border-gray-800">
                    <th className="h-8 sm:h-10 px-2 sm:px-4 text-left font-medium text-gray-400">
                      Backup ID
                    </th>
                    <th className="h-8 sm:h-10 px-2 sm:px-4 text-left font-medium text-gray-400">
                      Created
                    ...
                    <th className="h-8 sm:h-10 px-2 sm:px-4 text-right font-medium text-gray-400">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="text-gray-300">
                  {isLoading ? (
                    <tr>
                      <td colSpan={5} className="p-3 sm:p-4 text-center text-gray-300">
                        <div className="flex justify-center items-center space-x-2">
                          <RefreshCw className="h-4 w-4 animate-spin" />
                          <span className="text-xs sm:text-sm">Loading backups...</span>
                        </div>
                      </td>
                    </tr>
                  ) : backups.length === 0 ? (
                    ...
                    backups.map((backup) => (
                      <tr key={backup.id} className="border-b border-gray-800 hover:bg-gray-800/30">
                        <td className="p-2 sm:p-4 align-middle font-medium text-gray-300">
                          #{backup.id}
                        </td>
                        <td className="p-2 sm:p-4 align-middle text-gray-400">
