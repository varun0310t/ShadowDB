...
export async function POST(req: NextRequest) {
  try {
    ...
    const users: UserType[] = (await getDefaultReaderPool().query(`
      SELECT id, name, email 
      FROM users 
      WHERE email = $1
    `, [session.user.email])).rows;
    ...
    const payuData = {
      ...
      surl: `${WEBSITE_BASE_URL}/api/payments/success`,
      furl: `${WEBSITE_BASE_URL}/api/payments/failure`,
      hash: hash,
