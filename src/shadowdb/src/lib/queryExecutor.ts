...
export async function executeQuery(
  ...
  if (!pool) {
    try {
      ...
      const accessResult = await getDefaultReaderPool().query(accessQuery, accessValues);
      ...
      console.error(`Error accessing database ${db_name} for user ${userId}:`, error);
      throw error;
    }
  }
  ...
  try {
    ...
    queryCounter.inc({ userId, db_name, queryType });
    queryDurationHistogram.observe({ userId, db_name, queryType }, durationInSeconds);
    console.log(`Executed query for user: ${userId} on DB: ${db_name} (${queryType}) in ${durationInSeconds.toFixed(3)}s`);
